...
Brickrouge.Calendar.Editor = new Class({
	...
	initialize: function(options)
	{
		...
		for (y = type.length ; i < y ; i++)
		{
			...
			{
				case 'y':
					this.yearControl = control = new Element('input[type="text"][size="4"][name="year"]')
					break
				case 'm':
					this.monthControl = control = new Element('input[type="text"][size="4"][name="month"]')
					break
				case 'd':
